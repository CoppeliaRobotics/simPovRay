...
int ITextStream::getchar()
{
  ...
  {
    if(bufferoffset >= maxbufferoffset && ! RefillBuffer())
        return EOF;
...
// @CoppeliaSim@                                                                   //
//                                                                           //
// A new method to allow reading binary data                                 //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
    
bool ITextStream::get_bytes (void* v, int n)
{
    ungetbuffer = EOF;

    if (bufferoffset + n > maxbufferoffset)
        if (! RefillBuffer() || n > maxbufferoffset)
            return false;

    memcpy (v, buffer + bufferoffset, n);
    bufferoffset += n;
    return true;
}

bool ITextStream::RefillBuffer()
{
  if(bufferoffset < maxbufferoffset)
  {
