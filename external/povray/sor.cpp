 ...
 *---------------------------------------------------------------------------
 * $File: //depot/povray/3.6-release/source/sor.cpp $
 * $Revision: #3 $
 * $Change: 3032 $
...
static void Sor_UVCoord (UV_VECT Result, OBJECT *Object, INTERSECTION *Inter);
...
static int test_hit (SOR *, RAY *, ISTACK *, DBL, DBL, int, int);
...
static METHODS Sor_Methods =
{
  ...
  Inside_Sor, Sor_Normal, Sor_UVCoord,
...
static int intersect_sor(RAY *Ray, SOR *Sor, ISTACK *Depth_Stack)
{
  ...
  if (Test_Flag(Sor, CLOSED_FLAG) && (fabs(D[Y]) > EPSILON))
  {
    ...
    if (Sor->Base_Radius_Squared > DEPTH_TOLERANCE)
    {
      ...
      {
        if (test_hit(Sor, Ray, Depth_Stack, k / len, k, BASE_PLANE, 0))
        {
          found = true;
    ...
    if (Sor->Cap_Radius_Squared > DEPTH_TOLERANCE)
    {
      ...
      {
        if (test_hit(Sor, Ray, Depth_Stack, k / len, k, CAP_PLANE, 0))
        {
          found = true;
  ...
  {
#ifdef SOR_EXTRA_STATS
    if ( found ) Increase_Counter(stats[Sor_Bound_Tests_Succeeded]);
#endif
    return(found);
  }
...
*/
    ...
    while (n--)
    {
      ...
      {
        if (test_hit(Sor, Ray, Depth_Stack, k / len, k, CURVE, intervals[j].n))
        {
          found = true;
...
static void Translate_Sor(OBJECT *Object, VECTOR, TRANSFORM *Trans)
{
...
static void Rotate_Sor(OBJECT *Object, VECTOR, TRANSFORM *Trans)
{
  Transform_Sor(Object, Trans);
}
...
static void Scale_Sor(OBJECT *Object, VECTOR, TRANSFORM *Trans)
{
  Transform_Sor(Object, Trans);
}
...
static int test_hit(SOR *Sor, RAY *Ray, ISTACK *Depth_Stack, DBL d, DBL k, int t, int  n)
{
  VECTOR IPoint;
  ...
  if ((d > DEPTH_TOLERANCE) && (d < Max_Distance))
  {
    ...
    if (Point_In_Clip(IPoint, ((OBJECT *)Sor)->Clip))
    {
      ...
      push_entry_i1_i2_d1(d, IPoint, (OBJECT *)Sor, t, n, k, Depth_Stack);
...
static void Sor_UVCoord(UV_VECT Result, OBJECT *Object, INTERSECTION *Inter)
{
  ...
  switch (Inter->i1)
  {
    case CURVE:
      ...
      h = 
       Sor->Spline->BCyl->height[Sor->Spline->BCyl->entry[Inter->i2].h2] -
       Sor->Spline->BCyl->height[Sor->Spline->BCyl->entry[Inter->i2].h1];

        /* change in v per segment... divide total v (1.0) by number of segments */
      v_per_segment = 1.0/(Sor->Number);

      /* now find the current v given the current y */
      Result[V] = (P[Y] - Sor->Spline->BCyl->height[Sor->Spline->BCyl->entry[Inter->i2].h1]) / h
    ...
    case BASE_PLANE:
      /*Result[V] = 0;*/
      Result[V] = sqrt(P[X]*P[X]+P[Z]*P[Z])/sqrt(Sor->Base_Radius_Squared)-1;
      break;

    case CAP_PLANE:
      /*Result[V] = 1;*/
      Result[V] = -sqrt(P[X]*P[X]+P[Z]*P[Z])/sqrt(Sor->Cap_Radius_Squared)+2;
      break;
  }
